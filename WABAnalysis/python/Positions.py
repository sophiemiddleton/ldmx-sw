...
class WabEvent:
    ...
    def loop(self):
        ...
        for i in range(nentWAB):
            ...
            for ih, particle in enumerate(self.simParticles1):
                ...
                if  particle.second.getProcessType() == 0:
                    partList = [idcode];
                #print(i, ih, idcode, particle.second.getProcessType(), particle.second.getPdgID() )
        """  
        for i in range(nentWAB):
        ...
        for i in range(nentWAB):
            ...
            for ih,hit in enumerate(self.recoilSimHits1):
                [x, y, z] = hit.getPosition();
                j = hit.getID() & 0xFFF;
                #print(j)
                if j==1 and hit.getTrackID()==1:
                    self.RecoilHits_x.append(x)
                    self.RecoilHits_y.append(y)
                    self.RecoilHits_z.append(z)   
        ...
        for i in range(nentWAB):
            ...
            for ih,hit in enumerate(self.hcalScoringPlaneHits1):
                ...
                if j==41 and hit.getTrackID()==1:
                    ...
                    r = math.sqrt(hit.getMomentum()[0]*hit.getMomentum()[0]+hit.getMomentum()[1]*hit.getMomentum()[1]+hit.getMomentum()[2]*hit.getMomentum()[2])
                    if(hit.getMomentum()[0]!=0):
                        self.Elec_HCALSPHits_phi.append(math.acos(hit.getMomentum()[2]/ r))
                ...
                if j==41 and hit.getTrackID()==2:
                    ...
                    r = math.sqrt(hit.getMomentum()[0]*hit.getMomentum()[0]+hit.getMomentum()[1]*hit.getMomentum()[1]+hit.getMomentum()[2]*hit.getMomentum()[2])
                    if(hit.getMomentum()[0]!=0):
                        self.Phot_HCALSPHits_phi.append(math.acos(hit.getMomentum()[2]/ r))
...
def main(options,args) : 
    ...
    n, bins, patches = ax.hist(sc.RecHits_x,
            bins=50, 
            range=(-800,800), 
            label="Recoil Sim Hits (x)")
   
    ax.set_xlabel('Recoil Sim Hit x')  
    plt.savefig("RecoilSimHitEvent_x.png")
    fig, ax = plt.subplots(1,1)
    n, bins, patches = ax.hist(sc.RecHits_y,
            bins=50, 
            range=(-800,800), 
            label="Recoil Sim Hits (y)")
   
    ax.set_xlabel('Recoil Sim Hit y')  
    plt.savefig("RecoilSimHitEvent_y.png")
    fig, ax = plt.subplots(1,1)
    n, bins, patches = ax.hist(sc.RecHits_z,
            bins=50, 
            range=(0,2000), 
            label="Recoil Sim Hits (z)")
    
    ax.set_xlabel('Recoil Sim Hit z')  
    plt.savefig("RecoilSimHitEvent_z.png")
